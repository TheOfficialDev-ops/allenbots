...
module.exports = async (client) => {
	...
	app.use(
		session({
			...
			secret: 'SDNQI*HEHY(EGB$FDQ^NHI&EUO:VN@OA:BD:OW', // Randomly Generated Secret (DO NOT SHARE!)
			resave: false,
		...
		async (req, res) => {
			...
			if (banned) {
				client.users.fetch(req.user.id).then(async (a) => {
					client.channels.cache.get(channels.login).send(
						new Discord.MessageEmbed()
							...
							.setColor('RED')
							.setDescription(
								`[**${a.username}**#${a.discriminator}](${siteURL}/user/${a.id}) tried to access the site while banned.`
							)
				...
				req.session.destroy(() => {
					res.json({
						login: false,
						message: 'You have been blocked from the botlist.',
						logout: true,
					});
					req.logout();
				});
				...
				client.users.fetch(req.user.id).then(async (a) => {
					client.channels.cache.get(channels.login).send(
						new Discord.MessageEmbed()
							...
							.setColor('GREEN')
							.setDescription(
								`[**${a.username}**#${a.discriminator}](${siteURL}/user/${a.id}) Logged in successfully.`
							)
							.addField('Username', a.username)
							.addField('User ID', a.id)
							.addField('User Discriminator', a.discriminator)
	...
	http.listen(80, () => {
		console.log('[allenbots]: Website running on 80 port.');
	});
    ...
    */
	console.log(`
      [===========================================]
                       allenbots
      https://github.com/TheOfficialDev-ops/allenbots
               Developed by The Official Dev
	...
	sleep(1050);
	console.clear();
	console.log('\x1b[36m%s\x1b[0m', '[allenbots]: General routers loading...');
	sleep(500);
	...
	console.log(
		'\x1b[36m%s\x1b[0m',
		'[allenbots]: Uptime system routers loading...'
	);
	sleep(500);
	...
	console.log(
		'\x1b[36m%s\x1b[0m',
		'[allenbots]: Profile system routers loading...'
	);
	sleep(500);
	...
	console.log(
		'\x1b[36m%s\x1b[0m',
		'[allenbots]: Code Share system routers loading...'
	);
	sleep(500);
	...
	console.log(
		'\x1b[36m%s\x1b[0m',
		'[allenbots]: Botlist system routers loading...'
	);
	sleep(500);
	...
	console.log(
		'\x1b[36m%s\x1b[0m',
		'[allenbots]: Serverlist system routers loading...'
	);
	sleep(500);
	...
	console.log(
		'\x1b[36m%s\x1b[0m',
		'[allenbots]: Admin Panel system routers loading...'
	);
	sleep(500);
	...
	/* Bot System */
	console.log(' ');
	console.log('\x1b[36m%s\x1b[0m', '[allenbots]: Bot system loading...');
